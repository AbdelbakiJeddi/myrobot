...
CONFIG       = 0x1A
GYRO_CONFIG  = 0x1B
GYRO_FS_SEL  = 0x00  # ±250°/s (131 LSB/°/s) - suitable for ground robots
INT_ENABLE   = 0x38
ACCEL_XOUT_H = 0x3B
ACCEL_YOUT_H = 0x3D
ACCEL_ZOUT_H = 0x3F
...
class MPU6050_Driver(Node):
    ...
    def timerCallback(self):
        try:
            ...
            # Accelerometer: ±2g range, convert to m/s²
            self.imu_msg_.linear_acceleration.x = acc_x * 9.81 / 16384.0
            self.imu_msg_.linear_acceleration.y = acc_y * 9.81 / 16384.0
            self.imu_msg_.linear_acceleration.z = acc_z * 9.81 / 16384.0
            # Gyroscope: ±250°/s range, convert to rad/s
            self.imu_msg_.angular_velocity.x = gyro_x / 131.0 * math.pi / 180.0
            self.imu_msg_.angular_velocity.y = gyro_y / 131.0 * math.pi / 180.0
            self.imu_msg_.angular_velocity.z = gyro_z / 131.0 * math.pi / 180.0
    ...
    def init_i2c(self):
        try:
            ...
            self.bus_.write_byte_data(DEVICE_ADDRESS, GYRO_CONFIG, GYRO_FS_SEL)
